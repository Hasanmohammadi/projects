...
import {
  ...
  usePutEditStayDescription,
} from 'Hooks/Stay';
import usePostAddStayDescription from 'Hooks/Stay/usePostAddStayDescription';
import { StayDescriptionI } from 'Types/Stay';
import { useState } from 'react';
import { Plus } from 'react-feather';
import { useForm } from 'react-hook-form';

import StayDescriptionModal from './StayDescriptionModal';
...
interface AddStayDescriptionPropsI {
  hotelId: string;
}

export default function AddStayDescription({
  hotelId,
}: AddStayDescriptionPropsI) {
  ...
  const onAddStayDescription = async () => {
    setOpen(false);
    reset();
    await getStayInformationAction();
  };

  const { EditStayDescriptionAction } = usePutEditStayDescription({
    onSuccess: () => {
      getStayInformationAction().catch((err) => console.log(err));
    },
  });

  const { addStayDescriptionAction } = usePostAddStayDescription({
    onSuccess: () => {
      onAddStayDescription().catch((err) => console.log(err));
    },
  });
  ...
  const onEditSubmit = ({
    ...
    const x = stayInformationData?.hotelDescription?.filter(
      (stay) => languageId !== stay.languageId,
    );

    EditStayDescriptionAction({
      hotelId,
      hotelDescriptionsDetailes: [
        ...(x?.map((stay) => ({
          languageId: stay?.languageId,
          longDescription: stay?.longDescription,
  ...
  const onDeleteName =
    ...
    () => {
      ...
      EditStayDescriptionAction({
        hotelId,
        hotelDescriptionsDetailes: x as StayDescriptionI[],
      });
    };
  ...
  return (
    <div>
      ...
      {stayInformationData?.hotelDescription?.map(
        ({
          id,
          languageName,
          longDescription,
          ...
          <div className="mt-10  w-full" key={id}>
